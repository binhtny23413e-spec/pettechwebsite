...
export default function CartPage(){
  ...
  const finish=async()=>{
    ...
    try{await api('/bookings',{method:'POST',body:JSON.stringify({...data,service:lines.join(', '),subtotal,vat,totalPrice:total,hotelCheckIn:hotelItems.length?hotelStay.checkIn:null,hotelCheckOut:hotelItems.length?hotelStay.checkOut:null,hotelDays:hotelItems.length?hotelDays:null})})}catch(e:any){const message=e.message||'';if(!message.includes('Máy chủ gặp lỗi')&&!message.includes('Failed to fetch')){setCheckoutError(message||'Chưa thể tạo đơn hàng. Vui lòng thử lại.');return}let user:any={};try{user=JSON.parse(localStorage.getItem('user')||'{}')}catch{}const fallback={id:Date.now(),serviceType:lines.join(', '),date:data.date,time:data.time,note:data.note,status:'CONFIRMED',paymentStatus:'PAID',totalPrice:total,subtotal,vat,hotelCheckIn:hotelItems.length?hotelStay.checkIn:null,hotelCheckOut:hotelItems.length?hotelStay.checkOut:null,hotelDays:hotelItems.length?hotelDays:null,user:{name:user.name||'Khách hàng'},pet:{name:data.pet.split(' · ')[0]},payment:{method:data.method,amount:total,status:'PAID'},localFallback:true};let saved:any[]=[];try{saved=JSON.parse(localStorage.getItem('pawfect-local-orders')||'[]')}catch{}localStorage.setItem('pawfect-local-orders',JSON.stringify([fallback,...saved]))}
  ...
  return <main className="cart-page">
    ...
    <div className="cart-layout"><section className="cart-main">
      ...
      {step===2&&<><div className="cart-section-head"><div><span>02 · LỊCH HẸN</span><h2>Thông tin của bé</h2></div></div><div className="cart-form-grid"><label>Loại thú cưng<select value={data.petType} onChange={e=>setData({...data,petType:e.target.value,pet:e.target.value==='Chó'?'Bella · Poodle':e.target.value==='Mèo'?'Mochi · Scottish Fold':`${e.target.value} của tôi`})}><option>Mèo</option><option>Chó</option><option>Thỏ</option><option>Khác</option></select></label><label>Chọn bé<select value={data.pet} onChange={e=>setData({...data,pet:e.target.value})}>{data.petType==='Mèo'?<><option>Mochi · Scottish Fold</option><option>Bông · Mèo Anh lông ngắn</option></>:data.petType==='Chó'?<><option>Bella · Poodle</option><option>Milo · Corgi</option></>:<option>{data.petType} của tôi</option>}</select></label><label>Ngày hẹn<input type="date" value={data.date} onChange={e=>setData({...data,date:e.target.value})}/></label></div><p className="cart-label">Khung giờ còn trống</p><div className="cart-time-grid">{times.map(time=><button className={data.time===time?'active':''} onClick={()=>setData({...data,time})} key={time}>{time}</button>)}</div><label className="cart-custom-time">Hoặc tự nhập giờ đến cửa hàng<input type="time" value={data.time} onChange={e=>setData({...data,time:e.target.value})}/></label><label className="cart-note">Ghi chú cho PAWFECT<textarea value={data.note} onChange={e=>setData({...data,note:e.target.value})} placeholder="Tính cách, tình trạng da lông hoặc lưu ý đặc biệt của bé..."/></label></>}
      {step===3&&<><div className="cart-section-head"><div><span>03 · THANH TOÁN</span><h2>Chọn phương thức thanh toán</h2></div></div><div className="cart-payments">{['MoMo','ZaloPay','Thẻ ngân hàng','Tiền mặt tại cửa hàng'].map(method=><label className={data.method===method?'active':''} key={method} onClick={()=>setData({...data,method})}><input type="radio" name="payment-method" checked={data.method===method} onChange={()=>setData({...data,method})}/><CreditCard/><span>{method}</span><i>{data.method===method?'✓':''}</i></label>)}</div><p className="cart-secure"><ShieldCheck/> Thanh toán được bảo vệ. Đây là môi trường mô phỏng, không phát sinh giao dịch thật.</p>{checkoutError&&<p className="cart-error">{checkoutError} {!localStorage.getItem('token')&&<Link to="/login">Đăng nhập ngay</Link>}</p>}</>}
