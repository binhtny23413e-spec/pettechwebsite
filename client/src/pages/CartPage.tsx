...
export default function CartPage(){
  ...
  const finish=async()=>{
    const lines=items.map(item=>`${item.name} · ${item.variant} × ${item.quantity}`);
    const snapshot={code:`PF-${Date.now().toString().slice(-6)}`,subtotal,vat,total,lines};
    try{await api('/bookings',{method:'POST',body:JSON.stringify({...data,service:lines.join(', '),subtotal,vat,totalPrice:total})})}catch{}
  ...
  return <main className="cart-page">
    ...
    <div className="cart-layout"><section className="cart-main">
      ...
      {step===3&&<><div className="cart-section-head"><div><span>03 · THANH TOÁN</span><h2>Chọn phương thức thanh toán</h2></div></div><div className="cart-payments">{['MoMo','ZaloPay','Thẻ ngân hàng','Tiền mặt tại cửa hàng'].map(method=><label className={data.method===method?'active':''} key={method}><input type="radio" checked={data.method===method} onChange={()=>setData({...data,method})}/><CreditCard/><span>{method}</span><i>{data.method===method?'✓':''}</i></label>)}</div><p className="cart-secure"><ShieldCheck/> Thanh toán được bảo vệ. Đây là môi trường mô phỏng, không phát sinh giao dịch thật.</p></>}
