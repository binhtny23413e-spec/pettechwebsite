...
export function Login({register=false}:{register?:boolean}) {
  ...
  const adminLogin=loginParams.get('role')==='admin';
  const staffLogin=loginParams.get('role')==='staff';
  const [form,setForm]=useState({name:'',phone:'',email:register?'':adminLogin?'admin@pettech.vn':staffLogin?'staff@pettech.vn':'customer@pettech.vn',password:register?'':'123456',confirm:''});
  const [error,setError]=useState('');
  const submit=async(e:FormEvent)=>{e.preventDefault();setError('');try{const data=await api(register?'/auth/register':'/auth/login',{method:'POST',body:JSON.stringify(form)});localStorage.setItem('token',data.token);localStorage.setItem('user',JSON.stringify(data.user));nav(data.user.role==='CUSTOMER'?'/':'/'+data.user.role.toLowerCase())}catch(e:any){setError(e.message)}};
