...
const currentServicePrices:Record<string,Record<string,number>>={
  grooming:{silver:119000,gold:219000,premium:319000},
  spa:{silver:119000,gold:299000,premium:419000},
  'pet hotel':{silver:289000,gold:389000,premium:519000}
};
const currentComboPrices:Record<string,number>={
  'combo sạch xinh':219000,
  'combo tỏa sáng':479000,
  'combo kỳ nghỉ an tâm':1349000,
  'combo khỏe & đẹp':249000,
  'combo premium day':739000
};
function syncLegacyOrderPrice(order:Order):Order|null{
  if(order.subtotal!=null&&order.vat!=null)return order;
  let subtotal=0;const activeLines:string[]=[];
  for(const raw of order.serviceType.split(',').map(x=>x.trim()).filter(Boolean)){
    const line=raw.toLowerCase();
    const combo=Object.entries(currentComboPrices).find(([name])=>line.includes(name));
    const quantity=Math.max(1,Number(line.match(/×\s*(\d+)/)?.[1]||1));
    if(combo){subtotal+=combo[1]*quantity;activeLines.push(raw);continue}
    if(line.includes('chăm sóc sức khỏe')||line.includes('đưa đón tận nơi'))continue;
    const serviceName=line.includes('pet hotel')?'pet hotel':line.includes('grooming')?'grooming':line.includes('spa')?'spa':'';
    if(!serviceName)return order;
    const tier=line.includes('premium')?'premium':line.includes('gold')?'gold':'silver';
    const serviceQuantity=serviceName==='pet hotel'?(order.hotelDays||quantity):quantity;
    subtotal+=currentServicePrices[serviceName][tier]*serviceQuantity;activeLines.push(raw);
  }
